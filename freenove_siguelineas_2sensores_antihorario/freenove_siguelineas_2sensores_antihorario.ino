...
//Modificación para el ASTI Challenge
int retraso = 10;
...
void loop() {
  ...
  {
    case 0:   //00
      //motorRun(TK_TURN_SPEED_LV4, TK_TURN_SPEED_LV1); //giro horario
      motorRun(TK_TURN_SPEED_LV1, TK_TURN_SPEED_LV4); //giro antihorario
      delay(retraso);
      break;
...
u8 getTrackingSensorVal() {
  ...
  trackingSensorVal = (digitalRead(PIN_TRACKING_CENTER) == 1 ? 1 : 0) << 1 | (digitalRead(PIN_TRACKING_RIGHT) == 1 ? 1 : 0) << 0;
